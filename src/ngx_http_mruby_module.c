...
// These files are generated by util/regexpcodegen.rb
#include "../mrbs/regexp_pcre.rb.c"
#include "../mrbs/string_pcre.rb.c"
...
static ngx_int_t ngx_http_mruby_preinit(ngx_conf_t *cf)
{
    ...
    code = ngx_http_mruby_mrb_code_from_string(cf->pool, &string_pcre_rb);
    if (code == NGX_CONF_UNSET_PTR) {
        return NGX_ERROR;
    }
    ngx_http_mruby_shared_state_compile(mmcf->state, code);
