...
#define NGX_HTTP_MRUBY_REGEXP_IGNORECASE         0x01
#define NGX_HTTP_MRUBY_REGEXP_EXTENDED           0x02
#define NGX_HTTP_MRUBY_REGEXP_MULTILINE          0x04
...
static int ngx_http_mruby_mruby_to_pcre_options(mrb_value options)
{
    ...
    if (mrb_nil_p(options)) {
        ...
        if (nopt & NGX_HTTP_MRUBY_REGEXP_IGNORECASE) coptions |= PCRE_CASELESS;
        if (nopt & NGX_HTTP_MRUBY_REGEXP_EXTENDED)   coptions |= PCRE_EXTENDED;
        if (nopt & NGX_HTTP_MRUBY_REGEXP_MULTILINE)  coptions |= PCRE_MULTILINE;
...
static int ngx_http_mruby_pcre_to_mruby_options(int coptions)
{
    ...
    if (coptions & PCRE_CASELESS) {
        options |= NGX_HTTP_MRUBY_REGEXP_IGNORECASE;
    }

    if (coptions & PCRE_EXTENDED) {
        options |= NGX_HTTP_MRUBY_REGEXP_EXTENDED;
    }

    if (coptions & PCRE_MULTILINE) {
        options |= NGX_HTTP_MRUBY_REGEXP_MULTILINE;
    }
...
static mrb_value ngx_http_mruby_regexp_pcre_initialize(mrb_state *mrb, mrb_value self)
{
    ...
    mrb_iv_set(mrb, self, mrb_intern_lit(mrb, "@options"), mrb_fixnum_value(ngx_http_mruby_pcre_to_mruby_options(coptions)));
...
static mrb_value ngx_http_mruby_regexp_pcre_match(mrb_state *mrb, mrb_value self)
{
    ...
    for (i = 1; i < 10; i++) {
        ...
        mrb_gv_set(mrb, mrb_intern(mrb, sym, 8), mrb_funcall(mrb, md, "[]", 1, mrb_fixnum_value(i)));
    }
...
void ngx_http_mruby_regex_class_init(mrb_state *mrb)
{
    ...
    mrb_define_const(mrb, class_re, "MULTILINE",  mrb_fixnum_value(NGX_HTTP_MRUBY_REGEXP_MULTILINE));
